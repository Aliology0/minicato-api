...
public class StartCaseCommandHandler : IRequestHandler<StartCaseCommand, Result<string>>
{
    private readonly IAppDbContext _db;
    private readonly ICurrentUserService _currentUser;
    private readonly INotificationService _notificationService;
    private readonly ILogger<StartCaseCommandHandler> _logger;

    public StartCaseCommandHandler(
        IAppDbContext db,
        ICurrentUserService currentUser,
        INotificationService notificationService,
        ILogger<StartCaseCommandHandler> logger)
    {
        _db = db;
    ...
    public async Task<Result<string>> Handle(StartCaseCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!IsTransitionAllowed(caseEntity.Status))
            return Result<string>.Failure(new Error(
                ...
                $"Cannot transition from {caseEntity.Status} to {CaseStatus.InProgress}."));
        ...
        if (caseEntity.CaseClientRequest?.ClientRequest is not null)
        {
            ...
            try
            {
                ...
                var statusName = Messages.Enums.GetCaseStatus(CaseStatus.InProgress);
        ...
        return Result<string>.Success($"Status updated to {CaseStatus.InProgress}.");
    }
